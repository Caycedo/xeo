...
func TestNewAPI(t *testing.T) {
	t.Run("Successful Initialization", func(t *testing.T) {
		api, err := NewAPI(WithToken("test-token"))
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}

		if api.token != "test-token" {
			t.Errorf("Expected token 'test-token', got '%s'", api.token)
		}
		if api.baseURL != "https://api.telegram.org/bottest-token/" {
			t.Errorf("Expected baseURL 'https://api.telegram.org/bottest-token/', got '%s'", api.baseURL)
		}
		if api.client == nil {
			t.Errorf("Expected non nil HTTP client', got '%v'", api.client)
		}
	})

	t.Run("Initialization Without Token", func(t *testing.T) {
		_, err := NewAPI()
		if err == nil {
			t.Fatal("Expected error when initializing without token, got nil")
		}
		expectedErr := "token must be provided"
	...
	t.Run("Custom Options", func(t *testing.T) {
		...
		api, err := NewAPI(
			...
			WithBaseURL("https://custom-api.telegram.org/bot"),
		...
		if api.baseURL != "https://custom-api.telegram.org/bottest-token/" {
			t.Errorf("Expected custom baseURL, got '%s'", api.baseURL)
		}
...
func TestInvalidOptions(t *testing.T) {
	t.Run("Empty Token", func(t *testing.T) {
		_, err := NewAPI(WithToken(""))
		if err == nil {
			t.Fatal("Expected error with empty token, got nil")
		}
		expectedErr := "applying option: token cannot be empty"
		if err.Error() != expectedErr {
			t.Errorf("Expected error message '%s', got '%s'", expectedErr, err.Error())
		}
	})
	t.Run("Empty Base URL", func(t *testing.T) {
		_, err := NewAPI(WithToken("test-token"), WithBaseURL(""))
		if err == nil {
			t.Fatal("Expected error with empty base URL, got nil")
		}
		expectedErr := "applying option: base URL cannot be empty"
		if err.Error() != expectedErr {
			t.Errorf("Expected error message '%s', got '%s'", expectedErr, err.Error())
		}
	})
	t.Run("Nil HTTP Client", func(t *testing.T) {
		_, err := NewAPI(WithToken("test-token"), WithHTTPClient(nil))
		if err == nil {
			t.Fatal("Expected error with nil HTTP client, got nil")
		}
		expectedErr := "applying option: HTTP client cannot be nil"
		if err.Error() != expectedErr {
			t.Errorf("Expected error message '%s', got '%s'", expectedErr, err.Error())
		}
	})
...
var (
	...
	api, _              = NewAPI(WithToken("1713461126:AAEV5sgVo513Vz4PT33mpp0ZykJqrnSluzM"))
